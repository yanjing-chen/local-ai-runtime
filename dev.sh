...
CHECK
    ...
    echo "===== STAGE 1 RUNTIME TEST ====="

    PYTHONPATH="$ROOT/src" \
      "$PY" "$ROOT/tests/runtime_manager_test.py"

    echo "===== RESULT ====="
    echo "STAGE 0 API       PASS"
    echo "RUNTIME INSTALL   PASS"
    echo "RUNTIME UPGRADE   PASS"
    echo "RUNTIME ROLLBACK  PASS"
    echo "SHA256 VERIFY     PASS"
    echo "LOCAL AI RUNTIME STAGE 1: PASS"
    ;;
  ...
  runtime-status)
    PYTHONPATH="$ROOT/src" \
      exec "$PY" "$ROOT/src/runtimectl.py" status
    ;;
